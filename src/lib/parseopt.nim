...
## This version has a minor modification in the next proc to handle the posix
## requirements that '-' is treated as stdin:
...
proc next*(p: var OptParser) {.rtl, extern: "npo$1".} =
  ...
  of '-':
    ...
    if p.cmd[i] == '-':
      ...
      p.kind = cmdArgument
      # Move back to pass the '-' as a file name: '-' is stdin for posix
      p.pos = parseWord(p.cmd, i-1, p.key.string)
    else:
      p.pos = i
      handleShortOption(p)
  else:
